...
const main = async () =>{
    ...
    wait = await txn.wait();
    console.log("NEW RSVP2:", wait.events[0].event, wait.events[0].args);

    txn = await rsvpContract.confirmAllAttendees(eventID);
    wait = await txn.wait();
    console.log("length:", wait.events.length);
   wait.events.forEach((event) => console.log("CONFIRMED", event.args.attendeeAddress));

    //DepositUnclaimed deposits
